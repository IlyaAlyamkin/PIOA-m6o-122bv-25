...
def _add_student(table: StudentTable) -> None:
    print("\nДобавление записи")
...
def _show_all_students(table: StudentTable) -> None:
    print("\nСписок записей")
    _print_records(table.select_record())
...
def _find_students_by_filter(table: StudentTable) -> None:
...
def _update_student(table: StudentTable) -> None:
...
def _delete_student(table: StudentTable) -> None:
    print("\nУдаление записи")
    student_id = _read_int("id записи для удаления: ")
    try:
        deleted = table.delete_record(student_id)
...
def run() -> None:
    ...
    while True:
        ...
        if action == "1":
            ...
            _show_all_students(table)

        elif action == "3":
            _find_students_by_filter(table)

        elif action == "4":
            _update_student(table)

        elif action == "5":
            _delete_student(table)
