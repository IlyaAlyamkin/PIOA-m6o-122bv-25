...
### `__main__.py`
Точка входа при запуске пакета как модуля (`python -m src.db`): импортирует `run` из `tui` и запускает её.
...
1. Скопируйте все файлы проекта, сохранив указанную выше структуру папок. Убедитесь, что в папках `src`, `src/db` и `src/db/backend` присутствуют файлы `__init__.py` (они могут быть пустыми).
2. Откройте терминал и перейдите в корневую папку проекта (ту, которая содержит папку `src`):

   `cd путь/к/pioa-m60-122bv-25`

3. Запустите приложение из корня проекта (папка, где лежит `src`):

   `python -m src.db`

   Альтернативно, находясь в папке `src/db`, можно выполнить:
